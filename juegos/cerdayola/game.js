...
let score = 0;
let gameOver = false;
const goalDistance = 10; // Metros o puntos para completar el juego
const pointsPerObstacle = 1; // Puntos que se sumarán por cada obstáculo evitado

// Cargar la imagen de fondo
const backgroundImg = new Image();
backgroundImg.src = 'img/fondo.jpg'; // Asegúrate de que la ruta sea correcta

// Cargar la imagen de la moto
const motoImg = new Image();
motoImg.src = 'img/cars.png'; // Asegúrate de que la ruta sea correcta

// Cargar la imagen del obstáculo
const obstacleImg = new Image();
obstacleImg.src = 'img/obstacle.png'; // Aquí debes incluir la imagen pixelart del obstáculo

let backgroundY1 = 0; // Primer fondo
let backgroundY2 = -canvas.height; // Segundo fondo (empieza fuera de la pantalla)
...
function draw() {
    ...
    // Dibujar los fondos en posiciones Y para que se muevan de forma continua
    ...
    // Desplazar los fondos hacia abajo
    backgroundY1 += 1;
    backgroundY2 += 1;

    // Si el primer fondo sale de la pantalla, lo reposicionamos
    if (backgroundY1 >= canvas.height) {
        backgroundY1 = -canvas.height;
    }

    // Si el segundo fondo sale de la pantalla, lo reposicionamos
